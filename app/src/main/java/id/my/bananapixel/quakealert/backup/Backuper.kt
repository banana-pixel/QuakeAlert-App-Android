package id.my.bananapixel.quakealert.backup
...
class Backuper(val context: Context) {
    ...
    private suspend fun applySubscriptions(subscriptions: List<Subscription>?) {
        ...
        subscriptions.forEach { s ->
            try {
                // Add to database
                val subscription = id.my.bananapixel.quakealert.db.Subscription(
                    id = s.id,
                    baseUrl = s.baseUrl,
                    topic = s.topic,
                    instant = s.instant,
        ...
        notifications.forEach { n ->
            try {
                val actions = if (n.actions != null) {
                    n.actions.map { a ->
                        id.my.bananapixel.quakealert.db.Action(
                            id = a.id,
                            action = a.action,
                            label = a.label,
                            clear = a.clear,
                ...
                val attachment = if (n.attachment != null) {
                    id.my.bananapixel.quakealert.db.Attachment(
                ...
                val icon = if (n.icon != null && !n.icon.url.isNullOrEmpty()) {
                    id.my.bananapixel.quakealert.db.Icon(
                        url = n.icon.url,
                ...
                repository.addNotification(id.my.bananapixel.quakealert.db.Notification(
                    id = n.id,
    ...
    private suspend fun applyUsers(users: List<User>?) {
        ...
        users.forEach { u ->
            try {
                repository.addUser(id.my.bananapixel.quakealert.db.User(
                    baseUrl = u.baseUrl,
                    username = u.username,
                    password = u.password
                ))
